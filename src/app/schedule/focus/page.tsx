    ...
    return (
      <div className="mx-auto max-w-[1400px] px-4 py-8 sm:px-6">
        <div className="text-center">
          ...
          <p className="text-text-tertiary">{"Не удалось загрузить расписание. Попробуйте обновить страницу позже."}</p>
        </div>
      </div>
    );
  }
  return (
    <div className="overflow-auto rounded-lg border border-border bg-bg-card" style={{ maxHeight: "100vh" }}>
            <table className="w-full border-collapse text-[20px]">
              <thead>
                <tr>
                  <th className="sticky top-0 z-10 w-10 border border-border bg-bg-tertiary px-3 py-2 text-center text-xs font-semibold uppercase tracking-[0.04em] text-text-secondary">День</th>
                    ...
                    {LESSONS.map((lesson, li) => {
                      ...
                      return (
                        <tr key={`${day.id}-${lesson}`}>
                          {li === 0 ? (
                            <td rowSpan={LESSONS.length} className="min-w-[44px] border border-border bg-accent-primary-light px-2 py-2 text-center text-[30px] font-bold text-accent-primary [writing-mode:vertical-rl]">
                              {day.name}
                            </td>
                          ) : null}
                          <td className="w-10 min-w-[10px] border border-border bg-bg-tertiary px-2 py-2 text-center font-semibold text-text-secondary">{lesson}</td>
                          {classList.map((cls) => {
                            const cell = getCell(cls.id, day.id, lesson);
                            return (
                              <td
                              ...
                              >
                                {cell && cell.subjectName ? (
                                  <div className="flex min-h-[50px] flex-col justify-center gap-0.5 px-2 py-1">
                                    ...
                                    {cell.classrooms.length > 0 ? <div className="text-[12px] text-text-tertiary">Каб. {cell.classrooms.join(", ")}</div> : null}
                                  </div>
                                ) : (
                                  <div className="flex min-h-[15px] items-center justify-center px-2 py-1 text-center text-text-tertiary/30">—</div>
                                )}
                              </td>
                            );
                          })}
